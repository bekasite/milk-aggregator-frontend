...
// Remove toast import from api.js - handle toasts in components only
// DO NOT import toast here as it's a client-side library
  ...
  api.put = function(url, data = {}, config = {}) {
    ...
    if (url.includes('/orders/') && url.includes('/assign')) {
      ...
      if (!deliveryId) {
        ...
        // Don't use toast here - throw error instead
    ...
    (error) => {
      if (error.response) {
        ...
        if (error.response.status === 401) {
          ...
          // Only use localStorage and window in browser
...
// These functions are client-only
export const authGet = async (url) => {
  if (!isBrowser) throw new Error('Cannot authenticate on server')
